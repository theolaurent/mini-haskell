...
let rec infer q env = function
  | Const _ -> (q, SForall([], Ty.constructor "cst" []))
  | Var x -> (q, List.assoc x env)
  | Abstr (x, a) ->
     let alpha = Var.fresh () in
     let q1 = (alpha, (BFlexible, SBot)) :: q in
     let (q2, sigma) = infer q1 ((x,SForall ([], (Ty.variable alpha))) :: env) a in
     let beta = Var.fresh () in
     let (q3, q4) = split q2 (fst (List.split q)) in
     q3, SForall (List.rev_append q4 [beta, (BFlexible, sigma)],
		  Ty.arrow (Ty.variable alpha) (Ty.variable beta))
  | App (a, b) ->
     ...
     q4, SForall (List.rev q5, Ty.variable beta)
  | Let (x, a1, a2) ->
